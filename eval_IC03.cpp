...
int main( int argc, char** argv )
{
   ...
   int err     = 0; //false prositives
    ...
    ifstream labels("./lex/ICDAR2003/lex_FULL.txt");
    string line;
    ...
    if (!GTdoc.LoadFile()) {
        ...
        cerr << "  ./eval_IC03 <XML_GT_FILE>" << endl;
        return 1;
    }
  ...
  t_cnn_load = ((double)getTickCount() - t_cnn_load) / getTickFrequency();
    ...
    while (GTroot) // foreach image
    {
        ...
        string in_imagename = string("data/ICDAR2003/SceneTrialTest/")+GTimagename;
        cout << in_imagename << endl;
        outfile << in_imagename << endl;
        string lex_fname = string("lex/ICDAR2003/")+GTimagename+string("_voc.txt");

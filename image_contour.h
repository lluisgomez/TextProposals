...
public:
  ...
  inline void set_point_empty_C4(int row, int col) {
    ...
    data[key] = EMPTY;
    if (col > 0 && data[key - 1] == INNER) // left
      data[key - 1] = CONTOUR;
    if (col < colsm && data[key + 1] == INNER) // right
      data[key + 1] = CONTOUR;
    if (row > 0 && data[key - cols] == INNER) // up
  ...
  inline void set_point_empty_C8(int row, int col) {
    ...
    bool left_ok = col > 0, right_ok = col < colsm,
        up_ok = row > 0, down_ok = row < rowsm;
    if (left_ok && up_ok && data[key - 1] == INNER) // left
      data[key - 1] = CONTOUR;
    if (col < rowsm && data[key + 1] == INNER) // right
      data[key + 1] = CONTOUR;
    if (row > 0 && data[key - cols] == INNER) // up
      data[key - cols] = CONTOUR;
    if (row < colsm && data[key + cols] == INNER) // down
      data[key + cols] = CONTOUR;
  ...
  inline void from_image_loop_C4(const uchar* img_data, const uchar* img_ptr,
    ...
    for (int row = 0; row < rows; ++row) {
      for (int col = 0; col < cols; ++col) {
        ...
        if (*img_ptr) {
          int key = row * cols + col;
          if ((col > 0              && *img_ptr != img_data[key - 1])
              || (col < colsm       && *img_ptr != img_data[key + 1])
              || (row > 0           && *img_ptr != img_data[key - cols])
              || (row < rowsm       && *img_ptr != img_data[key + cols]))
          { // contour
    ...
    for (int row = 0; row < rows; ++row) {
      for (int col = 0; col < cols; ++col) {
        ...
        if (*img_ptr) {
          ...
          bool left_ok = col > 0, right_ok = col < colsm,
              up_ok = row > 0, down_ok = row < rowsm;
          if ((left_ok                 && *img_ptr != img_data[key - 1])
              || (right_ok             && *img_ptr != img_data[key + 1])
              || (up_ok                && *img_ptr != img_data[key - cols])
              || (down_ok              && *img_ptr != img_data[key + cols])
              || (left_ok && up_ok     && *img_ptr != img_data[key - cols - 1])
              || (left_ok && down_ok   && *img_ptr != img_data[key + cols - 1])
              || (right_ok && up_ok    && *img_ptr != img_data[key - cols + 1])
              || (right_ok && down_ok  && *img_ptr != img_data[key + cols + 1])
              )
          { // contour
            // printf("row:%i, col:%i is contour!\n", row, col);
            *out_ptr = CONTOUR;
